...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			write_log('Could not open the XML file for parsing!');
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			write_log('Content import start error: '. $result->get_error_message());
	...
	public function restore_import_data_transient() {
		if ( $data = get_transient( 'pt_importer_data' ) ) {
			...
			add_action( 'pt-importer/restore_import_data_transient','restore_import_data_transient' );
