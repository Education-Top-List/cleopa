...
if(!$swatch_attrs = cleopa_get_options('nbcore_pa_swatch_style')){
    $swatch_attrs = array();
} else {
...
<div class="product-addtocart">
	<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( wp_json_encode( $available_variations ) ) ?>">
		<?php if ( ! empty( $available_variations ) ) : ?>
			<div class="variations">
				...
				foreach ( $attributes as $name => $options ) :
					$loop++; ?>
					<h5 class="label"><label><?php echo wc_attribute_label( $name ); ?></label></h5>
								...
								if ( is_array( $options ) ) {
									...
									if ( taxonomy_exists( $name ) ) {
										...
										foreach ( $terms as $term ) {
											...
											echo '<li data-optionvalue="' . esc_attr( $term->slug ) . '" data-toggle="tooltip" title="'.$term->name.'" class="swatch-item ' . $class . '  ' . esc_attr( $term->slug ) . '" ><span style="'.$style.'" >' . apply_filters( 'woocommerce_variation_option_name', $term->name ) . '</span></li>';
										}
									}
								}
								?>
                    ...
                    if ( sizeof( $attributes ) === $loop ) {
                        echo '<a class="reset_variations" href="#reset">' . esc_html__( 'Clear selection', 'cleopa' ) . '</a>';
                    }
                    ?>
				<?php endforeach;?>
			</div>
		...
		<?php else : ?>
			<p class="stock out-of-stock"><?php _e( 'This product is currently out of stock and unavailable.', 'cleopa' ); ?></p>
		<?php endif; ?>
