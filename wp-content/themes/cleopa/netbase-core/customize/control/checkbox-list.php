...
class Cleopa_Customize_Control_Checkbox_List extends WP_Customize_Control {
    ...
    public function enqueue() {
        ...
        if (!isset($enqueued)) {
            wp_enqueue_script(
                    'nb-checkbox-list', get_template_directory_uri() . '/assets/netbase/js/admin/checkbox-list.min.js', array('jquery'), Cleopa_VER, true
            );
            $enqueued = true;
        }
    }
        ...
        <?php endif; ?>
            ...
            <ul class="customize-control-checkbox-lists">
                <?php foreach ($this->choices as $value => $label) : ?>
                    <li>
                        <label>
                            <input type="checkbox" name="<?php echo esc_attr($value); ?>" <?php count($multi_values) ? checked($multi_values->$value) : ''; ?> /> 
